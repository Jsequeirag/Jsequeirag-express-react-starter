...
```bash
pnpm dev              # Development with tsx watch (reloads on changes)
pnpm dev:node         # Development with node --watch on dist/
pnpm start                # Production (requires previous build)
pnpm build            # Compile TypeScript to dist/
pnpm test                 # Run tests once
pnpm test:watch       # Tests in watch mode
pnpm test:coverage    # Coverage
pnpm db:generate      # Generate Prisma client
pnpm db:push          # Sync schema with MongoDB
